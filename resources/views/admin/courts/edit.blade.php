<x-admin-layout>
    <div class="mb-6">
        <h2 class="text-2xl font-bold text-gray-800">Edit Lapangan: {{ $court->name }}</h2>
        <p class="text-gray-600">Perbarui informasi atau ganti foto lapangan.</p>
    </div>
        ...
        <div class="p-6 bg-white border-b border-gray-200">
            ...
            <form action="{{ route('admin.courts.update', $court->id) }}" method="POST" enctype="multipart/form-data">
                ...
                <div class="mb-4">
                    ...
                    <input type="text" name="name" value="{{ old('name', $court->name) }}" class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline" required>
                </div>
                ...
                <div class="grid grid-cols-2 gap-4">
                    <div class="mb-4">
                        ...
                        <select name="type" class="shadow border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline">
                            <option value="lantai" {{ old('type', $court->type) == 'lantai' ? 'selected' : '' }}>Lantai Kayu/Semen</option>
                            <option value="karpet" {{ old('type', $court->type) == 'karpet' ? 'selected' : '' }}>Karpet Vinyl</option>
                        </select>
                    </div>

                    <div class="mb-4">
                        <label class="block text-gray-700 text-sm font-bold mb-2">Harga per Jam (Rp)</label>
                        <input type="number" name="price" value="{{ old('price', $court->price) }}" class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline" required min="0">
                    </div>
                </div>
                ...
                <div class="mb-6">
                    ...
                    <select name="is_active" class="shadow border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline">
                        <option value="1" {{ old('is_active', $court->is_active) == 1 ? 'selected' : '' }}>Aktif (Bisa Dibooking)</option>
                        <option value="0" {{ old('is_active', $court->is_active) == 0 ? 'selected' : '' }}>Non-Aktif (Sedang Maintenance)</option>
                    </select>
                </div>

                <div class="mb-6 border-t pt-4">
                    <label class="block text-gray-700 text-sm font-bold mb-2">Ganti Foto Lapangan (Opsional)</label>
                    
                    @if($court->image)
                        <div class="mb-3">
                            <p class="text-xs text-gray-500 mb-1">Foto saat ini:</p>
                            <img src="{{ asset('storage/' . $court->image) }}" class="h-32 w-auto rounded-lg border shadow-sm object-cover">
                        </div>
                    @endif
